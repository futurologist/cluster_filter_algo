...
cluster_filter <- function(E){
  ...
  while(n != n_1){
    n_1 <- NROW(X)
    Mean <- matrix(rep(1, length = NROW(X)), ncol = 1) %*% colMeans(X)
    X_M <- X - Mean
    Sigma_inv <- inv( (t(X_M) %*% X_M)/(n_1-1) )
    X <- X[ which(apply(X_M, 1, function(r){ r1 <- as.matrix(r); return(t(r1) %*% Sigma_inv %*% (r1) <= sgm) })), ]
    n <- NROW(X)
  }
  return(data.table(X))
}
